...
for (let i = 1; i <= 10; i++) {
  ...
  const blur = '0'; // No blur at all
  shadows.push(`${i}px ${i}px ${blur} ${color}`);
}

let layer = 1; // Start with 1 shadow
let direction = 'up';
let timer;

function updateShadows(count) {
  const shadowString = shadows.slice(0, count).join(', ');
  headers.forEach((el) => {
...
function animateShadows() {
  if (direction === 'up') {
    if (layer <= 10) {
      updateShadows(layer);
      layer++;
      timer = setTimeout(animateShadows, 200);
    } else {
      setTimeout(() => {
        direction = 'down';
        layer = 9;
    ...
    if (layer >= 1) {
      ...
      direction = 'up';
      layer = 1;
      animateShadows(); // restart immediately
    }
  }
}
